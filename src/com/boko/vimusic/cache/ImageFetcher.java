...
public class ImageFetcher extends ImageWorker {
	...
	protected Bitmap processBitmap(final String url) {
		...
		final File file = downloadBitmapToFile(mContext, url,
	 ...
	 */
	@Override
	protected String processImageUrl(final String artistName,
			final String albumName, final ImageType imageType) {
		switch (imageType) {
		case ARTIST:
			if (!TextUtils.isEmpty(artistName)) {
				if (PreferenceUtils.getInstance(mContext)
						.downloadMissingArtistImages()) {
					final Artist artist = Artist.getInfo(mContext, artistName);
					if (artist != null && artist.getAvatarUrl() != null
							&& artist.getAvatarUrl().length() > 0) {
		...
		case ALBUM:
			if (!TextUtils.isEmpty(artistName) && !TextUtils.isEmpty(albumName)) {
				if (PreferenceUtils.getInstance(mContext)
					...
					final Artist correction = Artist.getCorrection(mContext,
							artistName);
					if (correction != null) {
						final Album album = Album.getInfo(mContext,
								correction.getName(), albumName);
						if (album != null && album.getAvatarUrl() != null
								&& album.getAvatarUrl().length() > 0) {
	 ...
	 */
	public void loadAlbumImage(final String artistName, final String albumName,
			final String albumId, final ImageView imageView) {
		loadImage(generateAlbumCacheKey(albumName, artistName), artistName,
				albumName, albumId, imageView, ImageType.ALBUM);
	}
	...
	public void loadCurrentArtwork(final ImageView imageView) {
		loadImage(
				generateAlbumCacheKey(MusicUtils.getAlbumName(),
	...
	public void loadCurrentArtistImage(final ImageView imageView) {
		loadImage(MusicUtils.getArtistName(), MusicUtils.getArtistName(), null,
				null, imageView, ImageType.ARTIST);
	}
	 ...
	 * @param keyArtist
	 *            The key (artist name) used to find the album art to return
	 */
	 ...
	 *            The key (artist name) used to find the album art to return
	 * @param keyId
	 *            The key (album id) used to find the album art to return
	 */
	public Bitmap getCachedArtwork(final String keyAlbum,
			final String keyArtist, final String keyId) {
		if (mImageCache != null) {
			return mImageCache.getCachedArtwork(mContext,
					generateAlbumCacheKey(keyAlbum, keyArtist), keyId);
		}
		return getDefaultArtwork();
	}
	 ...
	 * @param artistName
	 *            The album artist in case we should have to download missing
	 *            artwork
	 * @return The album art as an {@link Bitmap}
	 */
	public Bitmap getArtwork(final String albumName, final String albumId,
			final String artistName) {
		// Check the disk cache
		Bitmap artwork = null;

		if (artwork == null && albumName != null && mImageCache != null) {
			artwork = mImageCache.getBitmapFromDiskCache(generateAlbumCacheKey(
	 ...
	 */
	public static final File downloadBitmapToFile(final Context context,
			final String urlString, final String uniqueName) {
	...
	/**
	 * Generates key used by album art cache. It needs both album name and
	 * artist name to let to select correct image for the case when there are
	 * two albums with the same artist.
	 * 
	 * @param albumName
	 *            The album name the cache key needs to be generated.
	 * @param artistName
	 *            The artist name the cache key needs to be generated.
	 * @return
	 */
	public static String generateAlbumCacheKey(final String albumName,
			final String artistName) {
		if (albumName == null || artistName == null) {
			return null;
		}
		return new StringBuilder(albumName).append("_").append(artistName)
				.append("_").append(Config.ALBUM_ART_SUFFIX).toString();
	}
}
