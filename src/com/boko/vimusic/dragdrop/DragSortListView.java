...
public class DragSortListView extends ListView {
	...
	public DragSortListView(final Context context, final AttributeSet attrs) {
		...
		mController = new DragSortController(this,
				R.id.edit_track_list_item_handle, DragSortController.ON_DOWN,
		...
		mController.setBackgroundColor(getResources().getColor(
				R.color.holo_blue_light_transparent));
		...
		if (height > 0) {
			...
			if (isHeadFoot) {
				...
				if (height > 0) {
					...
					final int hspec = MeasureSpec.makeMeasureSpec(0,
							MeasureSpec.UNSPECIFIED);
					final int wspec = ViewGroup.getChildMeasureSpec(
							mWidthMeasureSpec, getListPaddingLeft()
			...
			if (isHeadFoot) {
				...
				heights.child = ((ViewGroup) item).getChildAt(0)
						.getMeasuredHeight();
			}
		}
	}
	 ...
	 * 
	 * @param position
	 *            Position from which item was obtained.
	 * @param item
	 *            List item (usually obtained from {@link ListView#getChildAt()}
	 *            ).
	 * @param heights
	 *            Object to fill with heights of item.
	 */
	private void getItemHeights(final int position, final View item,
			final ItemHeights heights) {
		final boolean isHeadFoot = position < getHeaderViewsCount()
				|| position >= getCount() - getFooterViewsCount();
	 ...
	 *            ListView position.
	 * @param heights
	 *            Object to fill with heights of item at <code>position</code>.
	 */
	...
	private void dropFloatView(final boolean removeSrcItem) {
		...
		if (removeSrcItem) {
			...
			if (mDropListener != null && mFloatPos >= 0
					&& mFloatPos < getCount()) {
				final int numHeaders = getHeaderViewsCount();
				mDropListener
						.drop(mSrcPos - numHeaders, mFloatPos - numHeaders);
			}
	...
	private void adjustAllItems() {
		...
		final int end = Math.min(last - first, getCount() - 1
				- getFooterViewsCount() - first);
	...
	private void adjustItem(final int position, final View v,
			final boolean needsMeasure) {
		...
		final int maxNonSrcBlankHeight = mFloatViewHeight
	...
	private void doDragScroll(final int oldFirstExpPos,
			final int oldSecondExpPos) {
		if (mScrollY == 0) {
			return;
		}
	...
	private void measureFloatView() {
		if (mFloatView != null) {
			...
			int hspec;
			if (lp.height > 0) {
				hspec = MeasureSpec.makeMeasureSpec(lp.height,
						MeasureSpec.EXACTLY);
			} else {
	 ...
	 */
	public boolean startDrag(final int position, final int dragFlags,
	 ...
	 *         in progress.
	 */
	public boolean startDrag(final int position, final View floatView,
			final int dragFlags, final int deltaX, final int deltaY) {
