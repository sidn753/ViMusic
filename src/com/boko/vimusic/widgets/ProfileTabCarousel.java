 ...
 */
@SuppressLint("NewApi")
public class ProfileTabCarousel extends HorizontalScrollView implements
		OnTouchListener {
	...
	public ProfileTabCarousel(final Context context, final AttributeSet attrs) {
		...
		mTabShadowHeight = mResources
				.getDimensionPixelSize(R.dimen.profile_carousel_label_height);
	...
	protected void onMeasure(final int widthMeasureSpec,
		...
		final int tabWidth = Math.round(tabWidthScreenWidthFraction
				* screenWidth);
		...
		final int tabHeight = Math.round(screenWidth
				* tabHeightScreenWidthFraction)
				+ mTabShadowHeight;
		if (getChildCount() > 0) {
			...
			if (mEnableSwipe) {
				child.measure(MeasureSpec.makeMeasureSpec(TAB_COUNT * tabWidth
						+ (TAB_COUNT - 1) * seperatorPixels,
						MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(
						tabHeight, MeasureSpec.EXACTLY));
			} else {
		...
		ApolloUtils.doAfterLayout(this, new Runnable() {
			...
			public void run() {
				scrollTo(mCurrentTab == TAB_INDEX_FIRST ? 0
						: mAllowedHorizontalScrollLength, 0);
				updateAlphaLayers();
			}
		});
	}
	 ...
	 */
	@Override
	protected void onScrollChanged(final int x, final int y, final int oldX,
			final int oldY) {
	...
	public void restoreYCoordinate(final int duration, final int tabIndex) {
		...
		final ObjectAnimator tabCarouselAnimator = ObjectAnimator.ofFloat(this,
				"y", storedYCoordinate);
		tabCarouselAnimator.addListener(mTabCarouselAnimatorListener);
		tabCarouselAnimator.setInterpolator(AnimationUtils
	...
	private void updateAlphaLayers() {
		float alpha = mLastScrollPosition * MAX_ALPHA
	 ...
	 * @param artistName
	 *            The artist name used to find the cached artist image and used
	 *            to find the last album played by the artist
	 */
	 ...
	 *            The {@link Activity} to use
	 * @param albumName
	 *            The key used to find the cached album art
	 * @param artistName
	 *            The artist name used to find the cached artist image
	 */
	 ...
	 *            The {@link Activity} to use
	 * @param profileName
	 *            The key used to find the cached image for a playlist or genre
	 */
	 ...
	 *            The {@link Context} to use.
	 * @param albumName
	 *            The name of the album in the profile the user is viewing.
	 * @param artistName
	 *            The name of the album artist in the profile the user is
	 *            viewing.
	 */
