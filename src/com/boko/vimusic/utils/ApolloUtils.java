...
import com.boko.vimusic.ui.activities.ShortcutActivity;
import com.boko.vimusic.widgets.ColorPickerView;
...
public final class ApolloUtils {
	...
	/**
	 * Used to determine if the device is running Jelly Bean MR2 (Android 4.3)
	 * or greater
	 * 
	 * @return True if the device is running Jelly Bean MR2 or greater, false
	 *         otherwise
	 */
	...
	public static final boolean isOnline(final Context context) {
		...
		boolean state = false;
		final boolean onlyOnWifi = PreferenceUtils.getInstance(context)
				.onlyOnWifi();
	 ...
	 * 
	 * @param view
	 *            The {@link View} to copy the content description from.
	 */
	public static void showCheatSheet(final View view) {
		...
		final int estimatedToastHeight = (int) (48 * context.getResources()
		...
		if (showBelow) {
			...
			cheatSheet.setGravity(Gravity.BOTTOM | Gravity.CENTER_HORIZONTAL,
					viewCenterX - screenWidth / 2, displayFrame.bottom
							- screenPos[1]);
		}
		cheatSheet.show();
	}
	...
	public static final AlertDialog createOpenSourceDialog(final Context context) {
		...
		return new AlertDialog.Builder(context)
				...
				.setView(webView).setPositiveButton(android.R.string.ok, null)
				.create();
	}
	 ...
	 *            The shortcut name
	 * @param id
	 *            The ID of the artist, album, playlist, or genre
	 * @param mimeType
	 *            The MIME type of the shortcut
	 * @param context
	 *            The {@link Context} to use to
	 */
	public static void createShortcutIntent(final String displayName,
			final String artistName, final String id, final String mimeType,
			final Activity context) {
		try {
			final ImageFetcher fetcher = getImageFetcher(context);
			Bitmap bitmap = null;
			if (mimeType.equals(MediaStore.Audio.Albums.CONTENT_TYPE)) {
				bitmap = fetcher.getCachedBitmap(ImageFetcher
						.generateAlbumCacheKey(displayName, artistName));
			} else {
			...
			final Intent shortcutIntent = new Intent(context,
			...
			AppMsg.makeText(
					context,
					context.getString(R.string.pinned_to_home_screen,
							displayName), AppMsg.STYLE_CONFIRM).show();
		} catch (final Exception e) {
			Log.e("ApolloUtils", "createShortcutIntent", e);
			AppMsg.makeText(
					context,
					context.getString(
							R.string.could_not_be_pinned_to_home_screen,
							displayName), AppMsg.STYLE_ALERT).show();
		}
	}
