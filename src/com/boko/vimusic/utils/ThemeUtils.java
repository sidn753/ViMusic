...
public class ThemeUtils {
	 ...
	 */
	public static final String APOLLO_PACKAGE = "com.boko.vimusic";
	...
	public ThemeUtils(final Context context) {
		...
		try {
			...
			mResources = mPackageManager
					.getResourcesForApplication(mThemePackage);
		...
		mCurrentThemeColor = PreferenceUtils.getInstance(context)
				.getDefaultThemeColor(context);
		// Inflate the custom layout
		mActionBarLayout = LayoutInflater.from(context).inflate(
	 ...
	 * 
	 * @param resourceName
	 *            The name of the color to return. i.e. "action_bar_color".
	 * @return A new color from the theme resources.
	 */
	public int getColor(final String resourceName) {
		final int resourceId = mResources.getIdentifier(resourceName, "color",
				mThemePackage);
		try {
	 ...
	 * 
	 * @param app
	 *            The {@link Activity} used to set the theme.
	 */
	 ...
	 * 
	 * @param menuItem
	 *            The {@link MenuItem} to set.
	 * @param resourceColorName
	 *            The color theme resource key.
	 * @param resourceDrawableName
	 *            The drawable theme resource key.
	 */
	public void setMenuItemColor(final MenuItem menuItem,
			final String resourceColorName, final String resourceDrawableName) {
		...
		final Bitmap outBitmap = Bitmap.createBitmap(width, height,
		...
		final BitmapDrawable outDrawable = new BitmapDrawable(mResources,
				outBitmap);
	...
	public void setFavoriteIcon(final Menu favorite) {
		...
		if (MusicUtils.isFavorite()) {
			setMenuItemColor(favoriteAction, "favorite_selected",
					favoriteIconId);
		} else {
	 ...
	 *            The {@link Context} to use.
	 * @param search
	 *            The Menu used to find the "menu_search" action.
	 */
	 ...
	 *            The {@link Context} to use.
	 * @param search
	 *            The Menu used to find the "menu_shop" action.
	 */
	 ...
	 *            The {@link Context} to use.
	 * @param search
	 *            The Menu used to find the "add_to_homescreen" item.
	 */
	public void setAddToHomeScreenIcon(final Menu search) {
		final MenuItem pinnAction = search
				.findItem(R.id.menu_add_to_homescreen);
	 ...
	 * 
	 * @param actionBar
	 *            The {@link ActionBar} to use.
	 * @param resources
	 *            The {@link ThemeUtils} used to theme the background, title,
	 *            and subtitle.
	 * @param title
	 *            The title for the action bar
	 * @param subtitle
	 *            The subtitle for the action bar.
	 */
	...
	public void shopFor(final Context context) {
		...
		shopIntent.setData(Uri.parse(String.format(SEARCH_URI,
				Uri.encode(sApolloSearch))));
